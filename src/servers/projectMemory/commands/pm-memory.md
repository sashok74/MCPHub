...
- `save_relationship` - Save a directional relationship between two entities. UPSERT by (entity_from, entity_to, rel_type).
  ...
  - Example: `save_relationship(entity_from="Orders", entity_to="OrderItems", rel_type="contains")`
...
- `save_form_mapping` - Map template names to C++ classes, files, and database tables
...
- `save_glossary` - Define domain terminology
  - Required: term
...
- `save_module` - Assign entities to a functional module. UPSERT (safe to call repeatedly).
  ...
  - Example: `save_module(module_name="Orders", entities=["Order", "OrderItem", "OrderStatus"])`
...
- `set_entity_status` - Set the research coverage level for an entity. UPSERT by entity name.
  ...
  - Example: `set_entity_status(entity="Orders", coverage="full", notes="Schema, business logic, code, forms")`
...
mcp__project-memory__save_fact({
  entity: "Orders",
  fact_type: "schema",
  description: "Table stores orders with fields OrderNumber, OrderDate, Status",
  evidence: "dbmcp:get_table_schema",
  confidence: "verified"
})
...
mcp__project-memory__save_form_mapping({
  template: "OrderForm",
  class_name: "COrderDlg",
  file_path: "OrderDlg.cpp",
  main_table: "Orders",
  sql_file: "OrderSQL.h",
...
mcp__project-memory__save_process({
  ...
  description: "Process of creating and fulfilling customer orders",
  steps: JSON.stringify([
    "1. Create order document (Orders)",
    "2. Add order items (OrderItems)",
    "3. Calculate totals",
    "4. Approve with manager",
    "5. Send to fulfillment"
  ]),
  related_entities: JSON.stringify(["Orders", "OrderItems", "Products", "Customers"])
})

// Search for previous knowledge (with filters)
mcp__project-memory__search_facts({
  query: "order processing",
  entity_filter: "Orders",
  fact_type_filter: "business_rule",
  limit: 10
})
...
mcp__project-memory__save_relationship({
  entity_from: "Orders",
  entity_to: "OrderItems",
  rel_type: "contains",
  description: "Order contains line items",
  evidence: "Orders.OrderID -> OrderItems.OrderID FK"
})

// Get all relationships for an entity
mcp__project-memory__get_relationships({
  entity: "Orders",
  direction: "both"  // incoming + outgoing
})

// Update an existing fact (fix inaccuracy)
mcp__project-memory__update_fact({
  id: 42,
  description: "Updated description with correct details",
  confidence: "verified"
})

// Delete an outdated fact
mcp__project-memory__delete_fact({ id: 99 })

// List all known entities
mcp__project-memory__list_entities({ fact_type: "schema" })

// Group entities into module
mcp__project-memory__save_module({
  module_name: "Orders",
  entities: ["Order", "OrderItem", "OrderStatus", "OrderHistory"]
})

// Set research coverage
mcp__project-memory__set_entity_status({
  entity: "Orders",
  coverage: "full",
  notes: "Schema, business logic, creation, status transitions"
})
...
mcp__project-memory__bulk_save({
  ...
  glossary: [{term: "New term", entity: "NewTable", context: "..."}],
  relationships: [{entity_from: "NewTable", entity_to: "OtherTable", rel_type: "references"}]
})

// Check coverage gaps
mcp__project-memory__get_entity_status()  // all entities
mcp__project-memory__list_modules()       // all modules

mcp__project-memory__ask_business({
  question: "how does order approval work"
})
```
